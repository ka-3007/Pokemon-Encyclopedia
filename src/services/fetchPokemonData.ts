...
import { saveToIndexedDB } from '@/repository/indexDB';
...
const fetchPokemonData = async (pokemonName: string) => {
  ...
  if (data.species && data.species.url) {
    ...
    const pokemonData: PokemonModel = {
      ...
      description: japaneseFlavorText || speciesResponse.data.flavor_text_entries[0].flavor_text || '説明はありません',
      evolutionData: evolutionData,
      formNames: formNames,
    };
    // ポケモンデータをFirebaseに保存
    await setDoc(PokemonRepo.pokemonDocRef(pokemonName), pokemonData);
    // クライアントサイドで実行されている場合のみIndexedDBに保存
